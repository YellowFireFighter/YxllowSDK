...
uintptr_t RLSDK::FindLiveBallAddress() {
    // Safety check
    if (g_GameViewportClientAddress == 0) return 0;

    uintptr_t geFromViewport = memManager_.Read<uintptr_t>(
        g_GameViewportClientAddress + 0x2E8).value_or(0);
    if (geFromViewport == 0) return 0;
    ...
    for (int32_t i = count - 1; i >= 0; --i) {
        ...
        std::string name = obj.GetName(memManager_, gnames_);
        if (name != "Ball_TA") continue;

        float x = memManager_.Read<float>(obj.Address + 0x90).value_or(0.f);
        float y = memManager_.Read<float>(obj.Address + 0x94).value_or(0.f);
        float z = memManager_.Read<float>(obj.Address + 0x98).value_or(0.f);

        if (x > -4200.f && x < 4200.f &&
            y > -5300.f && y < 5300.f &&
            z > -100.f && z < 2200.f &&
            !(x == 0.f && y == 0.f && z == 0.f))
            return obj.Address;
...
SDK::ABall RLSDK::GetBall() {
    ...
    // In menu — single cheap read, no scanning
    if (g_GameViewportClientAddress == 0) return SDK::ABall(0);
    uintptr_t geFromViewport = memManager_.Read<uintptr_t>(
        g_GameViewportClientAddress + 0x2E8).value_or(0);
    if (geFromViewport == 0) {
        cachedBallAddress_ = 0;
        ballStillTicks_ = 0;
        return SDK::ABall(0);
    }
    ...
    if (cachedBallAddress_ != 0) {
        ...
        float z = memManager_.Read<float>(cachedBallAddress_ + 0x98).value_or(0.f);

        bool valid = x > -4200.f && x < 4200.f &&
            y > -5300.f && y < 5300.f &&
            z > -100.f && z < 2200.f;
        if (valid) {
            float dx = x - lastBallPos_.X;
            float dy = y - lastBallPos_.Y;
            float dz = z - lastBallPos_.Z;
            if (dx * dx + dy * dy + dz * dz < 1.0f) {
                if (++ballStillTicks_ > 120) {
                    cachedBallAddress_ = 0;
                    ballStillTicks_ = 0;
                }
            }
            ...
            if (cachedBallAddress_ != 0)
                return SDK::ABall(cachedBallAddress_);
        }
        else {
            cachedBallAddress_ = 0;
        }
    ...
    cachedBallAddress_ = FindLiveBallAddress();
    return SDK::ABall(cachedBallAddress_);
}
